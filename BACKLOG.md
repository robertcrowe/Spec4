...
| Tests that assert less than they claim, and a path no test reaches. Four tests name an event and still pass when it never happens, each under a turn's forbidden-`None` mutation: `test_agents.py::TestBrainstormer::test_non_vision_response_stays_in_progress`, `::TestBrainstormerUnparseableArtifact::test_failed_reask_leaves_no_dead_end_user_turn`, `::TestBrainstormerUnparseableArtifact::test_state_is_not_advanced_when_both_attempts_fail`, and `test_code_scanner_progress.py::TestCharsTotal::test_total_is_monotonic_across_the_handover`. The path is `code_scanner.run`'s recap fall-through, reached by 0 of 43 traced invocations | 4 tests, 1 path | `PHASE8_RECORD.md` §14.2, §16, §17; `CLEANUP_REPORT.md` §2.4a |
