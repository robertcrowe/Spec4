...
class TestTableProvenance:
    ...
    def test_created_earlier_then_read_is_clean(self):
        ...
        flagged = [
            f for f in check_table_provenance(g) if f.severity in ("high", "medium")
        ]
        assert flagged == []
    ...
    def test_created_after_read_is_medium(self):
        ...
        meds = [f for f in check_table_provenance(g) if f.severity == "medium"]
    ...
    def test_created_never_read_is_info_only(self):
        ...
        findings = check_table_provenance(g)
...
class TestEndpointProvenance:
    def test_consumed_without_producer_is_medium(self):
        ...
        findings = check_endpoint_provenance(g)
    ...
    def test_consumed_with_producer_is_clean(self):
        ...
        assert check_endpoint_provenance(g) == []
...
class TestFeatureCoverage:
    ...
    def test_uncovered_mvp_feature_is_high(self):
        ...
        findings = check_feature_coverage(g, self._AF)
        assert any(
    ...
    def test_no_features_no_findings(self):
        assert check_feature_coverage(_g([]), None) == []


class TestFormatAndEntry:
    def test_format_empty_when_only_info(self):
        assert format_advisory([SeamFinding("table", "info", "x")]) == ""
    ...
    def test_run_seam_check_surfaces_findings(self):
        ...
        with patch.object(_seam_check, "extract_graph", return_value=graph):
...
class TestDeclarationAlignment:
    ...
    def test_aligned_declaration_is_silent(self):
        ...
        assert check_declaration_alignment(g, phases, self._AF) == []
    ...
    def test_under_declaration_is_high(self):
        ...
        findings = check_declaration_alignment(g, phases, self._AF)
    ...
    def test_unimplemented_feature_is_not_also_over_declared(self):
        # `related_items` is implemented nowhere: check_feature_coverage owns
        # that finding; we must not also accuse phase 1 of over-declaring it.
        g = _g([_phase(1, covers_features=["meaning_search"])])
        phases = [self._decl(1, "meaning_search", "related_items")]
        assert check_declaration_alignment(g, phases, self._AF) == []
        assert any(
            "Related items" in f.message for f in check_feature_coverage(g, self._AF)
        )
    ...
    def test_no_features_no_findings(self):
        assert check_declaration_alignment(_g([]), [], None) == []
        assert check_declaration_alignment(_g([]), [], {"ai_features": []}) == []
...
class TestExtractGraphTransport:
    """Phase-6 conversion: extract_graph drains complete_stream internally,
    ...
    def test_forwards_response_format_and_parses_stream(self):
        ...
        with (
            ...
            graph = _seam_check.extract_graph([_phase(1)], None, {"model": "x"})
    ...
    def test_response_format_none_when_unsupported(self):
        ...
        with (
            ...
            _seam_check.extract_graph([_phase(1)], None, {"model": "x"})
    ...
    def test_counter_published_and_seeded_from_session_value(self):
        ...
        with (
            ...
            _seam_check.extract_graph([_phase(1)], None, {"model": "x"}, session)
