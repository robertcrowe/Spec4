...
from spec4.agents._stack_context import stack_for_deployer
...
def test_absent_stack_renders_nothing() -> None:
    assert stack_for_deployer(None) == ""
    assert stack_for_deployer({}) == ""


def test_bare_and_wrapped_shapes_both_work() -> None:
    wrapped = stack_for_deployer(_stack())
    bare = stack_for_deployer(_stack()["stack_spec"])
...
def test_present_but_empty_auth_is_also_a_trustworthy_negative() -> None:
    ...
    out = stack_for_deployer(_stack(security={"auth": []}))
    assert "no user accounts" in out
...
def test_present_integrations_suppress_the_negative() -> None:
    out = stack_for_deployer(_stack(integrations=[{"name": "SendGrid"}]))
