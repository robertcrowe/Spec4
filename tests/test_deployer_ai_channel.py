...
def test_no_ai_features_renders_nothing() -> None:
    assert ai_features_for_deployer(None) == ""
    assert ai_features_for_deployer({}) == ""
    assert ai_features_for_deployer(_features()) == ""


def test_works_without_a_stack() -> None:
    """Tiers and budgets still render when no stack is supplied."""
    out = ai_features_for_deployer(_features(_node()))
...
def test_provider_renders_model_family_role_and_tiers() -> None:
    ...
    out = ai_features_for_deployer(_features(_node()), stack)
...
def test_catalog_provider_strategy_is_not_rendered() -> None:
    ...
    out = ai_features_for_deployer(spec, _stack(Anthropic={"model_family": "Claude"}))
...
def test_bare_and_wrapped_stack_shapes_both_work() -> None:
    ...
    wrapped = ai_features_for_deployer(_features(_node()), {"stack_spec": spec})
    bare = ai_features_for_deployer(_features(_node()), spec)
...
def test_budgets_render_latency_and_cost_per_feature() -> None:
    ...
    out = ai_features_for_deployer(_features(node))
...
def test_eval_and_safety_are_counted_not_quoted() -> None:
    ...
    out = ai_features_for_deployer(_features(*nodes))
