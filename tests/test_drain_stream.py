...
from spec4.agents._reask import drain_stream


def test_accumulates_and_returns_total() -> None:
    session: dict[str, Any] = {}
    text, total = drain_stream(iter(["ab", "cd", "e"]), session=session)
    assert text == "abcde"
    assert total == 5
...
def test_seed_offsets_total() -> None:
    session: dict[str, Any] = {}
    text, total = drain_stream(iter(["abc"]), session=session, seed=100)
    assert text == "abc"
    assert total == 103
...
def test_seed_published_eagerly_overwriting_stale_value() -> None:
    ...
    drain_stream(iter([]), session=session, seed=7)
