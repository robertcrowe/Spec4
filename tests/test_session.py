...
class TestDefaultSession:
    def test_has_all_expected_keys(self):
        ...
        required = [
            "working_dir", "browser_path", "specmem", "phase",
            "provider", "model", "api_key", "available_models",
            "tavily_api_key", "setup_error", "agent_select_error", "llm_config",
            "messages", "active_agent",
            "reviewer_state", "reviewer_messages", "code_review",
            "brainstormer_state", "brainstormer_messages", "vision_statement",
            "stack_advisor_messages", "stack_advisor_state", "stack_statement",
            "phaser_state", "phaser_messages", "phases",
            "_warn_existing_content", "_dir_has_content", "_initial_turn_done",
        ]
        for key in required:
...
class TestRunAgentBlocking:
    def _session(self, agent):
        return {"active_agent": agent, "llm_config": {"model": "gpt-4o", "api_key": "sk-test"}}

    def test_routes_to_brainstormer(self):
        session = self._session("brainstormer")
        with patch("spec4.session.brainstormer.run", return_value=iter(["hello"])) as mock_run:
    ...
    def test_routes_to_reviewer(self):
        ...
        with patch("spec4.session.reviewer.run", return_value=iter(["review"])) as mock_run:
            _run_agent_blocking("hi", session)
        mock_run.assert_called_once()

    def test_routes_to_stack_advisor(self):
        session = self._session("stack_advisor")
        with patch("spec4.session.stack_advisor.run", return_value=iter(["stack"])) as mock_run:
            _run_agent_blocking("hi", session)
        mock_run.assert_called_once()

    def test_routes_to_phaser_for_unknown_agent(self):
        session = self._session("phaser")
        with patch("spec4.session.phaser.run", return_value=iter(["phase"])) as mock_run:
            _run_agent_blocking("hi", session)
        mock_run.assert_called_once()

    def test_joins_generator_chunks(self):
        session = self._session("brainstormer")
        with patch("spec4.session.brainstormer.run", return_value=iter(["he", "ll", "o"])):
...
class TestPersistArtifacts:
    ...
    def test_saves_vision_when_complete(self):
        vision = {"name": "App"}
        session = self._base_session(brainstormer_state="vision_complete", vision_statement=vision)
        with patch("spec4.session.project_manager") as mock_pm:
            _persist_artifacts(session)
        mock_pm.save_vision.assert_called_once_with("/some/dir", vision)

    def test_does_not_save_vision_when_state_in_progress(self):
        session = self._base_session(brainstormer_state="in_progress", vision_statement={"name": "App"})
        with patch("spec4.session.project_manager") as mock_pm:
            _persist_artifacts(session)
        mock_pm.save_vision.assert_not_called()
    ...
    def test_saves_stack_when_complete(self):
        ...
        session = self._base_session(stack_advisor_state="stack_complete", stack_statement=stack)
        with patch("spec4.session.project_manager") as mock_pm:
            _persist_artifacts(session)
    ...
    def test_saves_code_review_when_complete(self):
        review = {"code_review": {}}
        session = self._base_session(reviewer_state="review_complete", code_review=review)
        with patch("spec4.session.project_manager") as mock_pm:
            _persist_artifacts(session)
