...
class TestDirHelpers:
    def test_get_spec4_dir_returns_dotspec4_subdir(self, tmp_path):
        assert project_manager.get_spec4_dir(tmp_path) == tmp_path / ".spec4"

    def test_ensure_spec4_dir_creates_directory(self, tmp_path):
        d = project_manager.ensure_spec4_dir(tmp_path)
        assert d.exists() and d.is_dir()

    def test_ensure_spec4_dir_is_idempotent(self, tmp_path):
...
class TestLoadArtifacts:
    ...
    def test_ignores_invalid_json_files(self, tmp_path):
    ...
    def test_ignores_invalid_phase_json(self, tmp_path):
        spec4_dir = project_manager.ensure_spec4_dir(tmp_path)
...
class TestSaveArtifacts:
    def test_save_vision_writes_json_file(self, tmp_path):
        vision = {"name": "MyApp"}
        project_manager.save_vision(tmp_path, vision)
    ...
    def test_save_stack_writes_json_file(self, tmp_path):
        stack = {"language": "Python"}
        project_manager.save_stack(tmp_path, stack)
    ...
    def test_save_code_review_writes_json_file(self, tmp_path):
        review = {"code_review": {}}
        project_manager.save_code_review(tmp_path, review)
    ...
    def test_save_creates_spec4_dir_if_missing(self, tmp_path):
...
class TestSpecmem:
    ...
    def test_update_replaces_existing_planning_section(self, tmp_path):
        s1 = {"vision_statement": {"name": "V1"}, "stack_statement": None, "phases": []}
        project_manager.update_specmem_planning_state(tmp_path, s1)
        s2 = {"vision_statement": {"name": "V2"}, "stack_statement": None, "phases": []}
    ...
    def test_update_includes_phases_summary(self, tmp_path):
        session = {
