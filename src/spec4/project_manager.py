...
def load_spec4_artifacts(working_dir: str | Path) -> dict:
    """Load vision.json, stack.json, code_review.json, and phases/*.json from .spec4/."""
    spec4_dir = get_spec4_dir(working_dir)
    result: dict = {"vision": None, "stack": None, "code_review": None, "phases": []}
...
def save_vision(working_dir: str | Path, vision: dict) -> None:
    spec4_dir = ensure_spec4_dir(working_dir)
    (spec4_dir / "vision.json").write_text(json.dumps(vision, indent=2), encoding="utf-8")


def save_stack(working_dir: str | Path, stack: dict) -> None:
    spec4_dir = ensure_spec4_dir(working_dir)
    (spec4_dir / "stack.json").write_text(json.dumps(stack, indent=2), encoding="utf-8")


def save_code_review(working_dir: str | Path, review: dict) -> None:
    spec4_dir = ensure_spec4_dir(working_dir)
    (spec4_dir / "code_review.json").write_text(json.dumps(review, indent=2), encoding="utf-8")


def save_phases(working_dir: str | Path, phases: list[dict]) -> None:
    spec4_dir = ensure_spec4_dir(working_dir)
    phases_dir = spec4_dir / "phases"
    phases_dir.mkdir(exist_ok=True)
    for phase in phases:
        num = phase.get("phase_number", 0)
        (phases_dir / f"phase{num}.json").write_text(json.dumps(phase, indent=2), encoding="utf-8")
...
def update_specmem_planning_state(working_dir: str | Path, session: dict) -> None:
