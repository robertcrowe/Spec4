...
def run(
    ...
    target_version, is_greenfield, is_revision = _phaser_round_flags(session)
    ...
    if user_input is None:
        if messages:
            ...
            return
        seed = _phaser_seed_message(session, target_version, is_revision)
    ...
    additions, last_text = _phaser_turn_additions(
        session, messages, pre_len, target_version
    )

    phases, failures, coverage_advisories = _phaser_validate(
        last_text, session, target_version, is_revision
    )

    if phases and failures:
        retry_user_msg, response_format, status_line = _phaser_retry_prompt(
            messages, failures, llm_config
        )
        yield status_line
        set_status(
            session,
        ...
        for _chunk in llm.stream_turn(
            ...
            _received = _phaser_count_chunk(session, _chunk, _received)
        phases, failures, coverage_advisories = _phaser_validate(
            last_assistant_text(messages), session, target_version, is_revision
        )
        if failures:
            _phaser_retry_exhausted(session, messages, failures, retry_user_msg)
            return

    if phases and not failures:
        _phaser_completion_marker(phases, target_version)
        display = _phaser_ready_display(phases, coverage_advisories)
        display = _phaser_seam_advisory(
            display, phases, session, llm_config, is_greenfield
        )
        _phaser_commit_phases(session, messages, phases, target_version, display)
    elif not phases and ("```json" in last_text or '"phase_number"' in last_text):
        _phaser_json_recovery_note(session, messages)


def _phaser_count_chunk(session: dict[str, Any], chunk: str, received: int) -> int:
    """Publish the cumulative received-character total for the retry drain (D-PH9)."""
    if chunk:
        received += len(chunk)
...
def _phaser_round_flags(session: dict[str, Any]) -> tuple[int, bool, bool]:
    """The round's version pin and its greenfield / revision gates."""
    ...
    return target_version, is_greenfield, is_revision
...
def _phaser_artifact_blocks(
    ...
    """AI-features, product-spine, design-note and manifest blocks."""
    ai_features_block = (
        ai_features_for_phaser(
            ai_features,
    ...
    return ai_features_block, spine_block, design_note_block, manifest_block


def _phaser_vision_and_stack_blocks(
    vision: Any, stack: Any, feature_specs: Any
) -> tuple[str, str]:
    """The vision paste (D-PH7a) and the stack spec plus its digest."""
    ...
    return vision_block, stack_block
...
def _phaser_revision_instruction(
    ...
    extra_block = f"{extra_block}{build_revision_note(cast(dict[str, Any], delta))}\n\n"
    instruction = (
...
def _phaser_seed_message(
    session: dict[str, Any], target_version: int, is_revision: bool
) -> str:
    """The whole seed message for a fresh Phaser turn."""
    ...
    ai_features_block, spine_block, design_note_block, manifest_block = (
        _phaser_artifact_blocks(
            ai_features, feature_specs, working_dir, target_version, is_revision
        )
    )
    vision_block, stack_block = _phaser_vision_and_stack_blocks(
        vision, stack, feature_specs
    )
    extra_block, instruction = _phaser_review_instruction(code_review)
    if is_revision:
        extra_block, instruction = _phaser_revision_instruction(extra_block, delta)
    round_block = (
...
def _phaser_turn_additions(
    session: dict[str, Any],
    messages: list[dict[str, Any]],
    pre_len: int,
    target_version: int,
) -> tuple[list[dict[str, Any]], str]:
    """Capture and persist human-confirmed stack additions from this turn."""
    ...
    return additions, last_text


def _phaser_validate(
    last_text: str, session: dict[str, Any], target_version: int, is_revision: bool
) -> tuple[list[dict[str, Any]], list[Any], list[str]]:
    """Extract, schema-validate, completeness-check and coverage-check a phase set."""
    ...
    return phases, failures, coverage_advisories


def _phaser_retry_prompt(
    messages: list[dict[str, Any]], failures: list[Any], llm_config: dict[str, Any]
) -> tuple[str, dict[str, Any] | None, str]:
    """The truncation note, the retry message, the response format and the status line."""
...
def _phaser_retry_exhausted(
    ...
    """The retry also failed: drop the correction exchange, surface the specifics."""
...
def _phaser_completion_marker(
    phases: list[dict[str, Any]], target_version: int
) -> None:
    """Append the set-completion marker instruction to the last phase."""
...
def _phaser_ready_display(
    phases: list[dict[str, Any]], coverage_advisories: list[str]
) -> str:
    """The phases-ready message, plus the not-built-by-these-phases note."""
...
def _phaser_seam_advisory(
    ...
    """Advisory cross-phase seam check, greenfield only; never blocks."""
...
def _phaser_commit_phases(
    ...
    """Commit the phase set to the session and set the display override."""
...
def _phaser_json_recovery_note(
    session: dict[str, Any], messages: list[dict[str, Any]]
) -> None:
    """Recoverable message when JSON-ish text parsed into no phase."""
