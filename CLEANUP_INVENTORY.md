...
4. **`layouts/designer.py:629`: `active_version(working_dir, session)` called with `working_dir: str | None`** (mypy `arg-type`, the one Phase 0.5b error left in place). When the session has no `working_dir` and no pinned `phase_version`, `active_version` falls through to `latest_phase_version(None)` → `get_spec4_dir(None)` → `Path(None)`, which raises `TypeError`. `_design_dir` on the next line already accepts `None`, so the surrounding code expects the no-project case to render. Fixing it needs a guard (a logic change), so it is logged here rather than fixed. Mypy is ratcheted to 1 error until this is decided.
